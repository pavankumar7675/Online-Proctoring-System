...
- Added passive liveness + anti-spoof scoring in the backend frame pipeline, including model live/spoof/missing counters.
- Frontend live overlay now shows per-frame counters above video (head, gaze, passive liveness, anti-spoof model stats).
- Real-time capture pipeline now keeps only the latest queued frame while one frame is in-flight to reduce stale-frame lag.
- Capture/watchdog timers are cleared on stop, reset, and unmount for safer session lifecycle behavior.
- Session state reset handling is centralized in backend session APIs to keep liveness and spoofing stats consistent.
...
- Node.js 20.19+ recommended (Vite 7 requirement)
- Windows/Linux/macOS
...
Install backend dependencies from the existing `requirements.txt` file:

```bash
pip install -r requirements.txt
```

Optional: enable ONNX anti-spoof runtime (if you plan to load an ONNX anti-spoof model):
...
- **Latest-Frame Queueing**: Drops stale queued frames when processing lags so UI stays near real-time
...
- **Passive Liveness + Anti-Spoof**: Live/suspicious/spoof indicators with model status and scores
- **Live Overlay Counters**: Above-video counters for head, gaze, passive liveness, and anti-spoof model results
...
- **Standalone mode (`app.py`)**: Processes every 10th frame with `MAX_FRAMES` cap for offline analysis
- **Web mode (`server.py`)**: Processes streamed frames in real-time; frontend throttles capture and keeps only latest queued frame while one is in-flight
- First 3 analyzed frames are used for baseline calibration
- Remaining frames are analyzed for violations and liveness signals
- Runs multiple AI models/signals in one pipeline for comprehensive monitoring

### 6. Passive Liveness and Anti-Spoof

- Computes passive liveness score from quality, blink, motion, and anti-spoof signals
- Supports optional ONNX anti-spoof inference (`ANTI_SPOOF_MODEL_PATH`) when configured
- Falls back to heuristic anti-spoof behavior when ONNX runtime/model is unavailable
- Tracks live session counters for:
   - passive labels (`live`, `suspicious`, `spoof`, `quality_insufficient`)
   - anti-spoof model labels (`live`, `spoof`, `missing`)
